...
Date.getTimezoneOffset = function(s, dst) {
    return dst || !1 ? Date.CultureInfo.abbreviatedTimeZoneDST[s.toUpperCase()] : Date.CultureInfo.abbreviatedTimeZoneStandard[s.toUpperCase()];
};

Date.getTimezoneAbbreviation = function(offset, dst) {
    var n = dst || !1 ? Date.CultureInfo.abbreviatedTimeZoneDST : Date.CultureInfo.abbreviatedTimeZoneStandard, p;
...
Date._validate = function(value, min, max, name) {
    ...
    if (value < min || value > max) throw new RangeError(value + " is not a valid value for " + name + ".");
    return !0;
};
...
Date.prototype.set = function(config) {
    var x = config;
    !x.millisecond && x.millisecond !== 0 && (x.millisecond = -1);
    !x.second && x.second !== 0 && (x.second = -1);
    !x.minute && x.minute !== 0 && (x.minute = -1);
    !x.hour && x.hour !== 0 && (x.hour = -1);
    !x.day && x.day !== 0 && (x.day = -1);
    !x.month && x.month !== 0 && (x.month = -1);
    !x.year && x.year !== 0 && (x.year = -1);
    x.millisecond != -1 && Date.validateMillisecond(x.millisecond) && this.addMilliseconds(x.millisecond - this.getMilliseconds());
    x.second != -1 && Date.validateSecond(x.second) && this.addSeconds(x.second - this.getSeconds());
    x.minute != -1 && Date.validateMinute(x.minute) && this.addMinutes(x.minute - this.getMinutes());
    x.hour != -1 && Date.validateHour(x.hour) && this.addHours(x.hour - this.getHours());
    x.month !== -1 && Date.validateMonth(x.month) && this.addMonths(x.month - this.getMonth());
    x.year != -1 && Date.validateYear(x.year) && this.addYears(x.year - this.getFullYear());
    x.day != -1 && Date.validateDay(x.day, this.getFullYear(), this.getMonth()) && this.addDays(x.day - this.getDate());
...
Date.prototype.getWeekOfYear = function(firstDayOfWeek) {
    var y = this.getFullYear(), m = this.getMonth(), d = this.getDate(), dow = firstDayOfWeek || Date.CultureInfo.firstDayOfWeek, offset = 8 - (new Date(y, 0, 1)).getDay();
    offset == 8 && (offset = 1);
    var daynum = (Date.UTC(y, m, d, 0, 0, 0) - Date.UTC(y, 0, 1, 0, 0, 0)) / 86400000 + 1, w = Math.floor((daynum - offset + 7) / 7);
    if (w === dow) {
        y--;
        var prevOffset = 8 - (new Date(y, 0, 1)).getDay();
        prevOffset == 2 || prevOffset == 8 ? w = 53 : w = 52;
    }
    return w;
};
...
(function() {
    ...
    var $P = Date.Parsing, _ = $P.Operators = {
        rtoken: function(r) {
            return function(s) {
                var mx = s.match(r);
        ...
        sequence: function(px, d, c) {
            d = d || _.rtoken(/^\s*/);
            c = c || null;
            return px.length == 1 ? px[0] : function(s) {
        ...
        set: function(px, d, c) {
            ...
            c = c || null;
            return function(s) {
                var r = null, p = null, q = null, rx = null, best = [ [], s ], last = !1;
                for (var i = 0; i < px.length; i++) {
                    q = null;
                    p = null;
                    r = null;
                    ...
                    !last && q[1].length === 0 && (last = !0);
                    if (!last) {
                        var qx = [];
                        for (var j = 0; j < px.length; j++) i != j && qx.push(px[j]);
...
(function() {
    var flattenAndCompact = function(ax) {
        var rx = [];
        for (var i = 0; i < ax.length; i++) ax[i] instanceof Array ? rx = rx.concat(flattenAndCompact(ax[i])) : ax[i] && rx.push(ax[i]);
        return rx;
    };
    Date.Grammar = {};
    Date.Translator = {
        ...
        finish: function(x) {
            ...
            for (var i = 0; i < x.length; i++) typeof x[i] == "function" && x[i].call(this);
            ...
            this.meridian && this.hour && (this.hour = this.hour < 13 && this.meridian == "p" ? this.hour + 12 : this.hour);
